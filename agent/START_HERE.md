...
Current state: Pete authorized implementation with EA integration set aside. Presence 0.1 is implemented locally; read `docs/09-local-prototype.md` first. This file does not authorize new paid services or deployment.
...
Completed local milestone: P0–P3 cover the persona interface, SQLite state, scripted conversation/tasks and verification. The next useful step is a real text-model adapter after checking API billing, secure credentials and a usage allowance with Pete. Original B0–B4 remain the broader backend roadmap; both EA/governance tasks B2a and B2b are deferred. Only perform paid/remote actions once the corresponding scope is authorized. Avatar A0–A2 can later proceed through verified desktop control without waiting for B9's automated runner.
