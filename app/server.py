...
class Worker(threading.Thread):
    def __init__(self, store, delay=0.7):
    ...
    def run(self):
        while not self._stopping.is_set():
            ...
            if self._stopping.wait(self.delay):
                break  # A restart will recover the unfinished claim.
            try:
                payload = task["payload"]
                if task["kind"] == "mock_reply":
...
def start_worker(store, delay=0.7):
    store.recover_pending()
    worker = Worker(store, delay)
    worker.start()
    return worker
    ...
    def do_GET(self):
        try:
            ...
            if path == "/api/state":
                ...
                self._json(200 if healthy else 503, {"status": "ok" if healthy else "error", "version": __version__, "database": "ok" if healthy else "error", "mode": "preview"})
    ...
    def do_POST(self):
        try:
            ...
            if path == "/api/persona":
                ...
                self._fields(payload, ("content", "request_id", "conversation_epoch"))
                if type(payload["conversation_epoch"]) is not int:
                    raise StoreError("conversation_epoch must be an integer.")
                self._json(202, {"task_id": self.server.store.enqueue_message(payload["content"], payload["request_id"], payload["conversation_epoch"])})
            elif path == "/api/tasks":
...
def create_server(store, port=8765):
    ...
    return PreviewServer(store, port)


def main(argv=None):
    parser = argparse.ArgumentParser(description="Start the local agent preview (no AI, no spending).")
    ...
    worker = None if args.no_worker else start_worker(store, args.worker_delay)
    print("Local preview ready: http://127.0.0.1:{}".format(server.server_address[1]), flush=True)
    print("Scripted preview only. No AI, voice, avatar, EA, remote tools, or paid services are connected.", flush=True)
    try:
